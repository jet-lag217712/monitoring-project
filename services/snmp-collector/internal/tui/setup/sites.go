...
func collectorIDForSiteID(siteID string) string {
	return "collector-development-vxrail-" + siteID
}
...
func BuildSiteSpecs(count int, siteIDs, cidrs []string) ([]SiteSpec, error) {
	...
	for i := 1; i <= count; i++ {
		...
		spec := SiteSpec{
			...
			MQTTClientID: mqttClientIDForSiteID(siteID),
...
func GenerateCompose(deployDir string, specs []SiteSpec, buildContext string) error {
