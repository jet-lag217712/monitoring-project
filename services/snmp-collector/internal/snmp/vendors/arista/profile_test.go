...
func TestProfileMetadata(t *testing.T) {
	...
	if p.GenericVendorPrefix() != "1.3.6.1.4.1.30065" {
...
func TestCollectSuccessFixture(t *testing.T) {
	...
	if got.Profile != "arista" || got.Capabilities != capabilities {
		t.Fatalf("metadata=%#v", got)
	}
	if got.CPU == nil || got.CPU.Value != 30 || got.CPU.SourceOID != oidProcessorLoad {
		t.Fatalf("CPU=%#v", got.CPU)
	}
	if got.Memory == nil || got.Memory.Value != 40 || got.Memory.SourceOID != oidStorageUsed {
		t.Fatalf("Memory=%#v", got.Memory)
	}
	if len(got.Temperatures) != 1 ||
		got.Temperatures[0].Name != "Board temperature" ||
		got.Temperatures[0].Value == nil ||
		*got.Temperatures[0].Value != 45 ||
		got.Temperatures[0].Unit != "celsius" ||
		got.Temperatures[0].Status != "ok" {
		t.Fatalf("Temperatures=%#v", got.Temperatures)
	}
	if len(got.Power) != 2 {
		t.Fatalf("Power=%#v", got.Power)
	}
	var foundWatts, foundSupply bool
	for _, component := range got.Power {
		switch component.Index {
		case 21:
			foundWatts = true
			if component.Value == nil || *component.Value != 120 || component.Unit != "watts" || component.Status != "ok" {
				t.Fatalf("watts component=%#v", component)
			}
		case 20:
			foundSupply = true
			if component.Name != "Power supply 1" || component.Status != "ok" || component.Value != nil {
				t.Fatalf("supply component=%#v", component)
			}
		}
	}
	if !foundWatts || !foundSupply {
		t.Fatalf("Power=%#v", got.Power)
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			check: func(t *testing.T, got readings.VendorReadings, err error) {
				...
				if got.CPU == nil || got.CPU.Value != 35 ||
		...
		},
		{
			...
			check: func(t *testing.T, got readings.VendorReadings, err error) {
				...
				if len(got.Temperatures) != 1 ||
					...
					got.Temperatures[0].Status != "critical" {
