...
ssh "${SSH_OPTS[@]}" debian@127.0.0.1 "sudo rm -rf ${STAGING} && sudo mkdir -p ${STAGING}/bundle"
tar -C "${BUNDLE_DIR}" -cf - . | ssh "${SSH_OPTS[@]}" debian@127.0.0.1 "sudo tar -xf - -C ${STAGING}/bundle"
scp "${SSH_OPTS[@]}" \
  "${SCRIPT_DIR}/configure-vm.sh" \
  "${SCRIPT_DIR}/prepare-ova.sh" \
  "${ROOT}/appliance/ci/provision-guest.sh" \
  debian@127.0.0.1:/tmp/equate-ci-scripts/
