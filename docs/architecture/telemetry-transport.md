...
Secure Local Telemetry Transport carries collector telemetry from the SNMP polling plane to the local Ingestion Service. MQTT/TLS is the current implementation; transport is never the system of record.
...
The boundary remains publish-only from collectors: no service gains a remote management path to the monitored network. TLS, authentication, least-privilege topic permissions, and secret redaction are mandatory.
