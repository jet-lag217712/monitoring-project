...
# Also test: delete while simulating sudo-from-ssh pattern
# Recreate user for SUDO_USER simulation
id debian >/dev/null 2>&1 || useradd -m debian
su -s /bin/bash debian -c "sleep 60" &
sleep 0.3
# kill other sessions first then force delete (proposed fix path)
loginctl terminate-user debian 2>/dev/null || true
...
echo "RC3=${RC3} EXISTS3=${EXISTS3} OUT3=${OUT3}"
...
print(json.dumps({
  ...
  "data": {
    ...
    "out_force": """${OUT2}""",
    "rc_pkill_then_force": ${RC3},
    "exists_pkill_then_force": "${EXISTS3}",
    "out_pkill_then_force": """${OUT3}""",
  },
